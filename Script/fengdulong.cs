...
public class fengdulong : MonoBehaviour {
   ...
   // public GameObject zidan;
    ...
    void Update()
    {
        ...
        if (stop == false)
        {
            if (jizhong == 0)
            {
                if ((Mathf.Abs(x - GameObject.Find("Robot Kyle").transform.position.x)) < range && (Mathf.Abs(z - GameObject.Find("Robot Kyle").transform.position.z)) < range)
                {
                    if ((Mathf.Abs(transform.position.x - GameObject.Find("Robot Kyle").transform.position.x)) < juli && (Mathf.Abs(transform.position.z - GameObject.Find("Robot Kyle").transform.position.z)) < juli)
                    {
                        ran = 1;
                   ...
                   //     gameObject.GetComponent<Animator>().SetInteger("run", 0);
                    ...
                    else
                    {
                        ...
                        transform.eulerAngles = new Vector3(0.0f, 90.0f + 90.0f - Mathf.Atan2(GameObject.Find("Robot Kyle").transform.position.z - transform.position.z, GameObject.Find("Robot Kyle").transform.position.x - transform.position.x) * 180 / Mathf.PI, 0.0f);
